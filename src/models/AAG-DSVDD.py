...
#
# Deep SVDD–style radius update:
#   R^2 := quantile_{1-ν}({ d_i^2 : i ∈ labeled normals })  — updated on a schedule
#
# Label convention:
#   +1 = labeled normal (used in labeled-hinge & center/R^2 updates)
...
class TrainConfig:
    ...
    # DeepSAD-style unlabeled pull-in
    ...
    # Deep SVDD–style R^2 update schedule
    r2_update_every: int = 1    # update R^2 every N epochs
    r2_start_epoch: int = 1     # start updating R^2 from this epoch
...
class AAGDeepSVDDTrainer:
    ...
    def _train_epoch(self, dl: DataLoader, y_all: torch.Tensor) -> float:
        ...
        for (xb, idxb) in dl:
            ...
            # Labeled hinges (Deep SVDD style scaling by 1/nu for normals)
            ...
            # DeepSAD-style unlabeled pull-in (with optional cap)
            if mask_u.any():
    ...
    def fit(self, X_all: torch.Tensor, y_all: torch.Tensor):
        ...
        # Initial R^2 from normals' (1 - nu) quantile (Deep SVDD style)
        with torch.no_grad():
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            # Rebuild graph on current embeddings (periodic)
            ...
            # Periodic R^2 update on labeled normals (Deep SVDD schedule)
...
if __name__ == "__main__":
    ...
    cfg = TrainConfig(
        ...
        r2_update_every=1, r2_start_epoch=1,     # Deep SVDD–style R^2 schedule
